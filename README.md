...
- `'record_type'`: `'run'`.
- `'record_time_mjd'`: the MJD associated with the GDF record. This is seemingly generated by the DAQ system `GRANITE`, possibly from the system time on the computer. I do not know how accurate it is.
...
- `'grs_data'`: raw data provided by GRS clock decoder. 
...
- `'trigger_data`': array of size `'ntrigger'` giving the trigger data words (I32).
